...
#=======================================================================
text_tokenizer_path = "./outputs/text_tokenizer.json"
summary_tokenizer_path = "./outputs/summary_tokenizer.json"
echo_log("Searching...", f"for saved tokenizer on path: {text_tokenizer_path}")
if os.path.exists(text_tokenizer_path):
    echo_log("Loading tokenizer ...", f"from: {text_tokenizer_path} and {summary_tokenizer_path}")
    with open(text_tokenizer_path, 'r', encoding='utf-8') as f:
        loaded_tokenizer_json = f.read()
        text_tokenizer = tokenizer_from_json(loaded_tokenizer_json)
    with open(summary_tokenizer_path, 'r', encoding='utf-8') as f:
        loaded_tokenizer_json = f.read()
        summary_tokenizer = tokenizer_from_json(loaded_tokenizer_json)
else:
    ...
    echo_log("Number of Vocab:",str(len(text_tokenizer.word_index)) + ", " + str(len(summary_tokenizer.word_index)))
    """
    echo_log("Reducing number of vocab...")
    #reduce words in tokenizer
    unique_words = ' '.join([word for word in tokenizer.word_index.keys() if word in tokenizer.word_counts.keys() and tokenizer.word_counts[word] >= 200])
    del tokenizer
    ...
    with open(text_tokenizer_path, 'w', encoding='utf-8') as f:
        f.write(tokenizer_json)
    echo_log("Saving tokenizer ...", f"json_file on => {text_tokenizer_path}")

    tokenizer_json = summary_tokenizer.to_json()
    with open(summary_tokenizer_path, 'w', encoding='utf-8') as f:
        f.write(tokenizer_json)
    echo_log("Saving tokenizer ...", f"json_file on => {summary_tokenizer_path}")
...
train_gen = TextSummaryWordLevelDataGenerator(data_frame=pd.DataFrame({"text": x_tr, "summary": y_tr}),
                                                text_tokenizer=text_tokenizer, summary_tokenizer=summary_tokenizer,
                                                max_text_len=max_text_len, max_summary_len=max_summary_len, batch_size=batch_size)
valid_gen = TextSummaryWordLevelDataGenerator(data_frame=pd.DataFrame({"text": x_val, "summary": y_val}),
                                                text_tokenizer=text_tokenizer, summary_tokenizer=summary_tokenizer,
                                                max_text_len=max_text_len, max_summary_len=max_summary_len, batch_size=batch_size)
